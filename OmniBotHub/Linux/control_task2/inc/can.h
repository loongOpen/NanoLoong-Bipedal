...
typedef struct
{
    char connect,connect_motor[14],connect_motor_servo[14];
    char ready[14],reset_imu,reset_q_div[14],reset_q_div_servo[14];
    int loss_cnt;
    char reset_q,reset_err;
    char motor_en,motor_en_reg;
    char servo_en;
    ...
    float q_servo[14],t_servo[14];
    float q_set_servo[14],q_set_servo_out[14],t_set_servo[14];
    float q_set_servo_init[14];
    float q_reset_servo[14];
    float q_set_servo_off[14];
    float q_servo_max[14],q_servo_min[14];
    float cap_scale[2];
    int servo_flag[14];
    float stiff_init;
    float stiff_stand;
    char servo_replay_mode;
}_LEG_MOTOR_ALL;
extern _LEG_MOTOR_ALL leg_motor_all,arm_motor_all;
