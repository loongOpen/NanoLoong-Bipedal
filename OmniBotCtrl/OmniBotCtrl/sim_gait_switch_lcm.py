...
lc = lcm.LCM("udpm://239.255.76.67:8888?ttl=1") # 0-本地 1-wifi
...
def my_handler(channel, data):
    global action_rl, action_count, last_print_time
    ...
    if elapsed >= 1.0:  # 每秒打印一次
        freq = action_count / elapsed
        # print(f"LCM_ACTION 实际接收频率: {freq:.2f} Hz")
        action_count = 0
        last_print_time = now
...
def run_mujoco(cfg):
    ...
    try:
        ...
        subscription = lc.subscribe("LCM_ACTION", my_handler)
        ...
        for step in range(total_steps):
            ...
            if count_lowlevel % cfg.sim_config.decimation == 0:
                ...
                #---send obs
                msg = Request()
                msg.eu_ang=eu_ang #姿态欧拉角
                msg.omega=omega #角速度
                msg.command[0]=cmd.vx #速度
                msg.command[1]=cmd.vy
                msg.command[2]=cmd.dyaw
                msg.q=q #关节位置
                msg.dq=dq #关节速度
                # 修改：使用键盘控制的步态状态
                msg.gait_type[0] = current_gait  # 使用当前步态状态
                lc.publish("LCM_OBS", msg.encode())
                #--LCM传输RL滤波后的输出 
                # 处理RL动作
