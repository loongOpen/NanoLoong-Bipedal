...
from modules import ActorCriticRMA,ActorCriticRmaTrans,ActorCriticSF,ActorCriticBarlowTwins,ActorCriticStateTransformer,ActorCriticTransBarlowTwins,ActorCriticMixedBarlowTwins
...
class OnConstraintPolicyRunner:
    ...
    def __init__(self,
        ...
        self.device = device
        self.env = env
        if self.env.cfg.env.en_logger == True:
            wandb.init()
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            if it % self.save_interval == 0:
                self.save(os.path.join(self.log_dir, 'model_{}.pt'.format(it))) #baocun
            ep_infos.clear()
    ...
    def log(self, locs, width=80, pad=35):
        ...
        if locs['ep_infos']:
            for key in locs['ep_infos'][0]:
                ...
                ep_string += f"""{f'Mean episode {key}:':>{pad}} {value:.5f}\n"""
        ...
        if len(locs['rewbuffer']) > 0:
            log_string = (f"""{'#' * width}\n"""
                          ...
                          f"""{'Value function loss:':>{pad}} {locs['mean_value_loss']:.5f}\n"""
                          f"""{'cost value function loss:':>{pad}} {locs['mean_cost_value_loss']:.5f}\n"""
                          f"""{'Surrogate loss:':>{pad}} {locs['mean_surrogate_loss']:.5f}\n"""
                          f"""{'viol loss:':>{pad}} {locs['mean_viol_loss']:.5f}\n"""
